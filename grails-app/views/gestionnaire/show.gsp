	...
	<body>
		...
		<div id="show-gestionnaire" class="content scaffold-show" role="main">
			...
			<ol class="property-list gestionnaire">
				...
				<g:if test="${gestionnaireInstance?.listeMusees}">
				<li class="fieldcontain">
					<span id="listeMusees-label" class="property-label"><g:message code="gestionnaire.listeMusees.label" default="Liste Musees" /></span>
					
						<g:each in="${gestionnaireInstance.listeMusees}" var="l">
						<span class="property-value" aria-labelledby="listeMusees-label"><g:link controller="musee" action="show" id="${l.id}">${l?.encodeAsHTML()}</g:link></span>
						</g:each>
